...
local function isInsideCoop(body)
    local plot = player:GetAttribute("Plot")
    local coops = Workspace:FindFirstChild("Coops")
    local model = plot and coops and coops:FindFirstChild("Coop" .. plot)
    local coop = dataClient:get({ "coop" })
    if not model or type(coop) ~= "table" then return false end

    local origin = model:GetAttribute("Origin") or model:GetPivot()
    local radius = coopView.size(tonumber(coop.slots) or 1, gameConfig.world.plotSize) / 2 + 2
    local offset = body.Position - origin.Position
    return Vector3.new(offset.X, 0, offset.Z).Magnitude <= radius
end
...
local function waitForChickenAtCoop(chickenId, healFull)
    ...
    return waitFor(function()
        ...
        for body, stats in chickenOverlay.bodies() do
            if roster and roster.activeId == chickenId and stats.owner == player.UserId then
                returning = stats.state == "corral"
                atCoop = returning
                    and isInsideCoop(body)
                    and (not healFull or (stats.hpFrac or 1) >= 0.999)
                if atCoop then break end
            end
        end

        if atCoop then
            stableAt = stableAt or now
            return now - stableAt >= 0.25
        end

        stableAt = nil
        if not returning and now >= nextOrder then
            chickenMode.order("coop")
            remotes.fire(remotes.defs.SetChickenOrder, "coop")
            nextOrder = now + 2
        end
...
local function prepareAndRebirth()
    ...
    -- Deploy the lowest-level backup for the rebirth swap.
    local deployed
    for _, c in chickens do
        if c.id ~= highest.id
            and (not deployed or (c.level or 1) < (deployed.level or 1))
        then
            deployed = c
        end
    end
    ...
    -- Deploy backup
    setStatus("Instant swap to small chicken...")
        ...
        if not state.busy and (state.enabled or state.restoreId) then
            ...
            elseif not isTowerActive() then
                ...
                if ready and state.enabled and state.mode == mode then
                    ...
                    elseif not waitFor(isTowerActive) then
                        setStatus("Tower start timeout")
                    end
                    state.busy = false
                end
            else
